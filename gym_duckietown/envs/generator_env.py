...
class GeneratorEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=0,
            high=255,
            shape=(self.params.CAMERA_HEIGHT, self.params.CAMERA_WIDTH, 3),
            dtype=np.uint8
        )
        ...
        self.text_label = pyglet.text.Label(
            ...
            font_size=14,
            x = 5,
            y = self.params.WINDOW_HEIGHT - 19
        )
        ...
        self.img_array_human = np.zeros(shape=(self.params.WINDOW_HEIGHT, self.params.WINDOW_WIDTH, 3), dtype=np.uint8)
        ...
        half_size = self.params.ROAD_TILE_SIZE / 2
        verts = [
    ...
    def reset(self, perturb_factor=1.0):
        """
        ...
        if self.domain_rand:
            ...
            if horz_mode == 0:
                self.horizon_color = self._perturb(self.params.BLUE_SKY_COLOR)
            elif horz_mode == 1:
                self.horizon_color = self._perturb(self.params.WALL_COLOR)
            elif horz_mode == 2:
                self.horizon_color = self._perturb(np.array([0.15, 0.15, 0.15]), 0.4)
            elif horz_mode == 3:
                self.horizon_color = self._perturb(np.array([0.9, 0.9, 0.9]), 0.4)
        else:
            self.horizon_color = self.params.BLUE_SKY_COLOR

        # Ground color
        self.ground_color = self._perturb(self.params.GROUND_COLOR, 0.3 * perturb_factor)

        # Distance between the robot's wheels
        self.wheel_dist = self._perturb(self.params.WHEEL_DIST)

        # Distance bewteen camera and ground
        self.cam_height = self._perturb(self.params.CAMERA_FLOOR_DIST, 0.08 * perturb_factor)

        # Angle at which the camera is pitched downwards
        self.cam_angle = self._perturb(self.params.CAMERA_ANGLE, 0.2 * perturb_factor)

        # Field of view angle of the camera
        self.cam_fov_y = self._perturb(self.params.CAMERA_FOV_Y, 0.2 * perturb_factor)
        ...
        for tile in self.grid:
            ...
            tile['color'] = self._perturb(self.params.ROAD_COLOR, 0.2)
        ...
        while True:
            ...
            x = self.np_random.uniform(i, i + 1) * self.params.ROAD_TILE_SIZE
            z = self.np_random.uniform(j, j + 1) * self.params.ROAD_TILE_SIZE
    ...
    def _load_map(self, map_name):
        ...
        for desc in map_data.get('objects', []):
            ...
            pos = self.params.ROAD_TILE_SIZE * np.array((x, 0, z))
    ...
    def _get_grid_coords(self, abs_pos):
        ...
        x, _, z = abs_pos
        i = math.floor(x / self.params.ROAD_TILE_SIZE)
        j = math.floor(z / self.params.ROAD_TILE_SIZE)
    ...
    def _get_curve(self, i, j):
        ...
        pts += np.array([(i+0.5) * self.params.ROAD_TILE_SIZE, 0, (j+0.5) * self.params.ROAD_TILE_SIZE])
    ...
    def _valid_pose(self):
        ...
        r_vec = self.get_right_vec()
        l_pos = self.cur_pos - 0.5 * self.params.ROBOT_WIDTH * r_vec
        r_pos = self.cur_pos + 0.5 * self.params.ROBOT_WIDTH * r_vec
        f_pos = self.cur_pos + 0.5 * self.params.ROBOT_WIDTH * f_vec
    ...
    def _render_img(self, width, height, multi_fbo, final_fbo, img_array):
        ...
        x, y, z = pos
        y += self.params.CAMERA_FLOOR_DIST
        dx, dy, dz = self.get_dir_vec()
        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()
        glRotatef(self.cam_angle, 1, 0, 0)
        glTranslatef(0, 0, self._perturb(self.params.CAMERA_FORWARD_DIST))
        gluLookAt(
            # Eye position
            x,
            y,
        ...
        for j in range(self.grid_height):
            for i in range(self.grid_width):
                ...
                glPushMatrix()
                glTranslatef((i+0.5) * self.params.ROAD_TILE_SIZE, 0, (j+0.5) * self.params.ROAD_TILE_SIZE)
    ...
    def render(self, mode='human', close=False):
        ...
        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()
        glOrtho(0, self.params.WINDOW_WIDTH, 0, self.params.WINDOW_HEIGHT, 0, 10)
