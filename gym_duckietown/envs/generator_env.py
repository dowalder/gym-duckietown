...
class GeneratorEnv(gym.Env):
    ...
    def reset(self, perturb_factor=0.0):
        """
        ...
        # Ground color
        self.ground_color = self._perturb(GROUND_COLOR, 0.3 * perturb_factor)

        # Distance between the robot's wheels
        self.wheel_dist = self._perturb(WHEEL_DIST, 0.1 * perturb_factor)

        # Distance bewteen camera and ground
        self.cam_height = self._perturb(CAMERA_FLOOR_DIST, 0.08 * perturb_factor)

        # Angle at which the camera is pitched downwards
        self.cam_angle = self._perturb(CAMERA_ANGLE, 0.2 * perturb_factor)

        # Field of view angle of the camera
        self.cam_fov_y = self._perturb(CAMERA_FOV_Y, 0.2 * perturb_factor)
    ...
    def _get_lookahead_point(self, grid_i, grid_j, lookahead_dist):
        cps = self._get_curve(grid_i, grid_j)
        t = bezier_closest(cps, self.cur_pos)
        point = bezier_point(cps, t)
        ...
        dot_dir = dot_dir / abs(dot_dir)
        new_point = point
        increment = 0.0
        while np.linalg.norm(point - new_point) < lookahead_dist:
            increment += dot_dir * 0.001
            new_point = bezier_point(cps, t + increment)
        return new_point
        ...
        # # Compute the signed angle between the direction and curve tangent
        # # Right of the tangent is negative, left is positive
        # angle = math.acos(dot_dir)
        # angle *= 180 / math.pi
        # if np.dot(dir_vec, rightVec) < 0:
        #     angle *= -1

    def lookahead_point(self, lookahead_dist=0.1):
        i, j = self._get_grid_coords(self.cur_pos)

        goal_point = self._get_lookahead_point(i, j, lookahead_dist)
        # i_new, j_new = self._get_grid_coords(goal_point)
        # if i_new != i or j_new != j:
        #     goal_point = self._get_lookahead_point(i_new, j_new, lookahead_dist)
        return goal_point

    def theoretical_step(self, action, delta_t=0.1, point=None) -> float:
        """
